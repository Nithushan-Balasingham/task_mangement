...
const AllTasks = () => {
  ...
  return (
   <div className="p-4">
    <h3 className="text-3xl text-green-400 mb-9 font-semibold mt-8 flex items-center justify-center">
  ...
  <thead className="text-xs text-gray-700 uppercase dark:bg-gray-700 dark:text-gray-400">
    <tr>
      <th scope="col" className="px-6 py-3 border-r border-gray-700">Title</th>
      <th scope="col" className="px-6 py-3 border-r border-gray-700">Description</th>
      <th scope="col" className="px-6 py-3 border-r border-gray-700">Priority</th>
      <th scope="col" className="px-6 py-3 border-r border-gray-700">Due Date</th>
  ...
  <tbody>
    {filteredTasks.map((item, index) => (
      <tr className="bg-white border-b border-gray-700 hover:bg-gray-50" key={index}>
        <th
          scope="row"
        ...
        <td className="px-6 py-4 border-r border-gray-700">{item.description}</td>
        <td className="px-6 py-4 border-r border-gray-700  ">
          <span
            className={`inline-block w-3 h-3 rounded-full mr-2  ${
              item.priority === 'low' ? 'bg-orange-400' :
              item.priority === 'medium' ? 'bg-yellow-400' :
              item.priority === 'high' ? 'bg-red-400' : ''
            }`}
          ></span>
          {item.priority ==="low" ? "Low" : item.priority == "medium"  ? <span className="font-semibold">Medium</span> :<span className="font-bold">High</span>}
        </td>
        <td className="px-6 py-4 border-r border-gray-700">{formatDate(item.due_date)}</td>
